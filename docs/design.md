...
## Deleting a Payment (Payments Page + Client Detail)
- Two entry points share the same reversal logic (`src/lib/paymentReversal.js` — `reverseTicketPaymentRow`, `reverseStandaloneSupplierRefund`), so they can't drift apart the way the old duplicated Client Detail version once did:
...
- A standalone supplier_refund payment (linked via payments.ticket_id, no ticket_payments row — see RefundModal/LogTransactionModal) reverses refund_received directly against that ticket. Only reachable from the Payments page today, since Client Detail never lists supplier-side payments
...
- Supplier Detail's Payment History tab has no delete action at all yet (client side only, matching the original design)
...
- Payment History tab row actions: View / Edit (opens ViewPaymentModal), Allocate (if unallocated_amount > 0), Delete (client side only — see "Deleting a Payment" below for the full reversal rules)
