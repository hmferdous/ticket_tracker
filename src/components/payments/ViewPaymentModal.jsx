...
export default function ViewPaymentModal({ isOpen, onClose, payment, onSaved }) {
  ...
  const partyBadgeCls = isClientSide ? "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400" : "bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 px-4 py-6" onMouseDown={handleBackdrop}>
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{editing ? "Edit Payment" : "Payment Details"}</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-800 dark:text-gray-200 text-sm">{party?.name ?? "—"}</span>
          </div>

          {!editing ? (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Date</p>
                <p className="text-gray-800 dark:text-gray-200">{fmtDate(payment.payment_date)}</p>
              </div>
              <div>
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Amount</p>
                <p className="text-gray-800 dark:text-gray-200 tabular-nums">{fmt(payment.amount)} BDT</p>
              </div>
              <div>
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Channel</p>
                <p className="text-gray-800 dark:text-gray-200">{payment.channel ?? "—"}</p>
              </div>
              <div>
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Trx ID</p>
                <p className="text-gray-800 dark:text-gray-200">{payment.trx_id ?? "—"}</p>
              </div>
              <div>
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Unallocated</p>
                <p className={`tabular-nums ${(payment.unallocated_amount ?? 0) > 0 ? "text-yellow-600 dark:text-yellow-400 font-medium" : "text-gray-800 dark:text-gray-200"}`}>
                  {fmt(payment.unallocated_amount)} BDT
                </p>
              </div>
              <div className="col-span-2">
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-1">Notes</p>
                <p className="text-gray-800 dark:text-gray-200">{payment.notes || "—"}</p>
              </div>
            </div>
          ) : (
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Allocated to Tickets</h3>
            {loading ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">Loading…</p>
            ) : allocations.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">No tickets allocated from this payment yet.</p>
            ) : (
              <div className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {allocations.map((a) => (
                      <tr key={a.id}>
                        <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{a.tickets?.passenger_name ?? "—"}</td>
                        <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{a.tickets?.route ?? "—"}</td>
                        <td className="px-3 py-2 text-right tabular-nums font-medium text-blue-700 dark:text-blue-400">{fmt(a.allocated_amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
