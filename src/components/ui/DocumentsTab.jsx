...
const DOC_TYPE_CLS = {
  "Business Card": "bg-blue-100 text-blue-700",
  "NID": "bg-orange-100 text-orange-700",
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      ...
      {docs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
          {docs.map((doc) => (
            <div key={doc.id} className="flex flex-col gap-2 border border-gray-200 rounded-xl p-4 hover:border-gray-300 transition-colors">
              ...
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${DOC_TYPE_CLS[doc.doc_type] ?? "bg-gray-100 text-gray-600"}`}>
                  {doc.doc_type}
                </span>
      ...
      {docs.length < MAX_DOCS && (
        <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
          ...
          <button
            ...
            disabled={uploading}
            className="flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700 font-medium border border-blue-200 hover:bg-blue-50 px-4 py-1.5 rounded-lg transition-colors disabled:opacity-50"
          >
            {uploading ? "Uploading…" : "+ Upload document"}
          </button>
          <input ref={fileInputRef} type="file" className="hidden" onChange={handleFileChange} />
          <span className="text-xs text-gray-400 ml-auto">{docs.length} / {MAX_DOCS}</span>
        </div>
      )}

      {docs.length >= MAX_DOCS && (
        <p className="text-xs text-gray-400 pt-4 border-t border-gray-100">Maximum of {MAX_DOCS} documents reached.</p>
      )}
    </div>
  )
}
