...
function computeTicketChips(ticket) {
  ...
  if (ticket.is_void) chips.push({ label: "Void", cls: "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400" })
  if (ticket.status === "reissued") chips.push({ label: "Reissued", cls: "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400" })
  if (ticket.is_reissue) chips.push({ label: "Reissue", cls: "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400" })
  if (ticket.refund_status != null && ticket.refund_status !== "closed") chips.push({ label: "Refund", cls: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400" })
  if (ticket.refund_status === "closed") chips.push({ label: "Refunded", cls: "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" })
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {loading ? (
          <div className="py-20 text-center text-sm text-gray-400 dark:text-gray-500">Loading client…</div>
        ) : !client ? (
          <div className="py-20 text-center text-sm text-gray-400 dark:text-gray-500">Client not found.</div>
        ) : (
          <>
            {/* Client details card */}
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 mb-6 flex items-start gap-4">
              <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 text-xs font-semibold tracking-wide mt-1.5">
                {clientIdLabel(client.client_id_number)}
              </span>
              <div>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{client.name}</h2>
                <div className="mt-2 flex flex-wrap gap-x-8 gap-y-1 text-sm">
                  ...
                  <p>
                    ...
                    <span className="text-gray-700 dark:text-gray-300">{client.notes || "—"}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              ...
              <StatCard label="Total Received" value={totalReceived} accent="text-green-600 dark:text-green-400" />
              <StatCard
                label="Outstanding Balance"
                value={outstandingBalance}
                accent={outstandingBalance > 0 ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-gray-100"}
              />
              <StatCard
                label="Unallocated Credit"
                value={unallocatedCredit}
                accent={unallocatedCredit > 0 ? "text-blue-600 dark:text-blue-400" : "text-gray-900 dark:text-gray-100"}
                action={
                  unallocatedCredit > 0 && (
                    <button
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 text-left">
                        ...
                        <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-right">Sell Price</th>
                        <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-right">Amount Paid</th>
                        <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-right">Outstanding</th>
                        <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Status</th>
                        <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Chips</th>
                      </tr>
                    </thead>
                      ...
                      {tickets.map((ticket) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right tabular-nums text-gray-700 dark:text-gray-300">{fmt(ticket.sell_price)}</td>
                            <td className="px-4 py-3 text-right tabular-nums text-gray-600 dark:text-gray-400">{fmt(ticket.amount_paid)}</td>
            ...
            {activeTab === "payments" && (
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
                {payments.length === 0 ? (
                  <div className="py-16 text-center text-sm text-gray-400 dark:text-gray-500">No payments logged for this client yet.</div>
                ) : (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 text-left">
